...
//  VideoEffects
//
//  Created by Johnny Xu(徐景周) on 7/22/15.
...
- (void)setPickedVideo:(NSURL *)url checkVideoLength:(BOOL)checkVideoLength
{
    ...
    if (checkVideoLength)
    {
        if (getVideoDuration(url) > MaxVideoLength)
        {
            ...
            NSString *error = GBLocalizedString(@"Reminder");
...
- (void)pickMusicFromCustom
{
    ...
    [audioController setSeletedRowBlock: ^(BOOL success, id result) {
        ...
        if (success && [result isKindOfClass:[NSNumber class]])
        {
            ...
            // Build video effect
            FilterType filterType = [self getFilterType];
            ThemesType curThemeType = KThemeOldFilm;
            if (filterType == kSwirl)
            {
                curThemeType = kThemeNostalgia;
            }
            else if (filterType == kMosaic)
            {
                curThemeType = kThemeMood;
            }
...
- (void)viewDidLoad
{
    ...
    if (appRunCount == 0)
    {
        ...
        [self setShouldDisplayTextEffects:YES];
        [self setShouldDisplayPloygon:YES];
        [self setFilterType:kDistortion];
//        [self setShouldDisplayGhost:YES];
    }
...
- (void)addAppRunCount
{
    ...
    {
        ++appRunCount;
        NSString *flag = @"AppRunCount";
        NSUserDefaults *userDefaultes = [NSUserDefaults standardUserDefaults];
        [userDefaultes setInteger:appRunCount forKey:flag];
...
- (NSUInteger)getAppRunCount
{
    ...
    NSString *flag = @"AppRunCount";
    NSUserDefaults *userDefaultes = [NSUserDefaults standardUserDefaults];
    if ([userDefaultes integerForKey:flag])
    {
        appRunCount = [userDefaultes integerForKey:flag];
    }
...
- (void)setShouldDisplayInnerBorder:(BOOL)shouldDisplay
{
    ...
    if (shouldDisplay)
    {
        [userDefaultes setBool:YES forKey:flag];
    }
    else
    {
        [userDefaultes setBool:NO forKey:flag];
    }
...
- (void)setShouldDisplayTextEffects:(BOOL)shouldDisplay
{
...
- (void)setShouldDisplayPloygon:(BOOL)shouldDisplay
{
...
- (void)setShouldDisplayGhost:(BOOL)shouldDisplay
{
...
#pragma mark - FilterType
- (void)setFilterType:(FilterType)filterType
{
    NSString *flag = @"FilterType";
    NSUserDefaults *userDefaultes = [NSUserDefaults standardUserDefaults];
